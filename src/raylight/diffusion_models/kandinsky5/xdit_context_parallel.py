...
def usp_dit_forward(self, x, timestep, context, y, freqs, freqs_text, transformer_options={}, **kwargs):
    ...
    freqs_text = torch.chunk(freqs_text, get_sequence_parallel_world_size(), dim=2)[get_sequence_parallel_rank()]
...
def usp_self_attn_foward(self, x, freqs, **kwargs):
    ...
    out = xfuser_optimized_attention(q, k, v, self.num_heads)
...
def usp_cross_attn_forward(self, x, context, transformer_options={}, **kwargs):
    ...
    out = xfuser_optimized_attention(self.query_norm(q), self.key_norm(k), v, self.num_heads)
    return self.out_layer(out)
